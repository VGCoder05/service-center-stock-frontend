...
class ClientExcelService {
  static addTitle(worksheet, title, dateRange) {
  ...
  static styleDataRow(row, category) {
  ...
  static async generateStockValuation(data, options = {}) {
    const { startDate, endDate } = options;
    const workbook = new ExcelJS.Workbook();
    workbook.creator = 'Service Center Stock Manager';

    const summarySheet = workbook.addWorksheet('Summary');
    this.addTitle(summarySheet, 'Stock Valuation Report', startDate && endDate ? `${startDate} to ${endDate}` : 'All Time');
    ...
    categories.forEach(([cat, info]) => {
      const row = summarySheet.addRow([
        cat.replace(/_/g, ' '), info.count, info.totalValue,
    ...
    for (const [category, items] of Object.entries(data.details)) {
      ...
      items.forEach(item => {
        const rowData = [
          item.serialNumber, item.partName, item.partCode || '-', item.voucherNumber,
          item.billDate ? new Date(item.billDate).toLocaleDateString('en-IN') : '-',
          item.supplierName || '-', item.unitPrice
        ];

        if (['SPU_PENDING', 'SPU_CLEARED'].includes(category)) {
          rowData.push(item.context?.spuId || '-', item.context?.customerName || '-', item.context?.isChargeable ? 'Yes' : 'No');
        } else if (category === 'AMC') {
          rowData.push(item.context?.customerName || '-', item.context?.amcNumber || '-');
        } else if (category === 'OG') {
          rowData.push(item.context?.customerName || '-', item.context?.cashAmount || 0, item.context?.paymentStatus || '-');
        } else if (category === 'RETURN') {
          rowData.push(item.context?.returnReason || '-');
        } else if (category === 'RECEIVED_FOR_OTHERS') {
          rowData.push(item.context?.receivedFor || '-', item.context?.transferStatus || '-');
        } else if (category === 'IN_STOCK') {
      ...
      const subtotalRow = sheet.addRow(['SUBTOTAL', '', '', '', '', '', items.reduce((sum, i) => sum + (i.unitPrice || 0), 0)]);
      subtotalRow.font = { bold: true };

      sheet.columns.forEach((col, i) => { col.width = i === 0 ? 20 : (i === 1 ? 25 : 15); });
      sheet.getColumn(7).numFmt = currencyFormat;
    }

    // RETURN A BUFFER INSTEAD OF SAVING A FILE
    return await workbook.xlsx.writeBuffer();
  }

  // Generate Bill-wise Report
  static async generateBillWiseReport(data, options = {}) {
    const { startDate, endDate } = options;
    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet('IN STOCK by Bill');
    this.addTitle(sheet, 'IN STOCK Report (Grouped by Bill)', startDate && endDate ? `${startDate} to ${endDate}` : 'All Time');
    ...
    data.forEach(bill => {
      ...
      bill.serials.forEach((serial, idx) => {
        const row = sheet.addRow([
          ...
          serial.serialNumber, serial.partName, serial.unitPrice, serial.context?.location || '-'
        ]);
    ...
    sheet.getColumn(1).width = 18; sheet.getColumn(2).width = 18; sheet.getColumn(3).width = 20;
    sheet.getColumn(4).width = 12; sheet.getColumn(5).width = 20; sheet.getColumn(6).width = 25;
    sheet.getColumn(7).width = 15; sheet.getColumn(7).numFmt = currencyFormat; sheet.getColumn(8).width = 15;
  ...
  // Generate SPU Report
  static async generateSPUReport(data, options = {}) {
...
export default ClientExcelService;